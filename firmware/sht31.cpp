//=========================================================================================================
// sht31.cpp - Implements an efficient class for reading the SHT31 temperature/humidity sensor
//=========================================================================================================
#include <avr/pgmspace.h>
#include "sht31.h"
#include "globals.h"

// Include the TWI library with C linkage
extern "C"
{
    #include <utility/twi.h>
};  


//=========================================================================================================
// 8-bit CRC lookup table for polynomial 0x31
//=========================================================================================================
static const uint8_t crc8_table[] PROGMEM =
{
    
};
//=========================================================================================================


//=========================================================================================================
// fast_crc8() - Uses the lookup table to generate an 8-bit CRC
//=========================================================================================================
static uint8_t fast_crc8(const uint8_t* in, uint8_t count)
{
    uint8_t crc = 0xFF;
    while (count--) crc = pgm_read_byte(crc8_table + (crc ^ *in++));
    return crc;
}
//=========================================================================================================



//=========================================================================================================
// raw_to_c() - Converts a raw reading to degrees C
//
// The formulate for converting raw to degrees C given in the datasheet is:
//
// Degrees C = (175 * raw) / 65535 - 45
//
// We can make that faster in floating point by computing 100ths of a degree C using integers:
//
// 100ths of a degree C = (100 * 175 * raw) / 65535 - 4500
//
// Which simplifies to:
//
// 100ths of a degree C = (17500 * raw) >> 16 - 4500
//
//=========================================================================================================
static float raw_to_c(uint32_t raw_temp)
{
    // Compute the temperature in 100ths of a degree C
    int16_t hundreths = ((17500 * raw_temp) >> 16) - 4500;

    // Return the temperature in degrees C
    return hundreths * .01F;
}
//=========================================================================================================




//=========================================================================================================
// raw_to_f() - Converts a raw reading to degrees F
//
// The formulate for converting raw to degrees F given in the datasheet is:
//
// Degrees F = (315 * raw) / 65535 - 49
//
// We can make that faster in floating point by computing 100ths of a degree F using integers:
//
// 100ths of a degree F = (100 * 315 * raw) / 65535 - 4900
//
// Which simplifies to:
//
// 100ths of a degree F = (31500 * raw) >> 16 - 4900
//
//=========================================================================================================
static float raw_to_f(uint32_t raw_temp)
{
    // Compute the temperature in 100ths of a degree F
    int16_t hundreths = ((31500 * raw_temp) >> 16) - 4900;

    // Return the temperature in degrees F
    return hundreths * .01F;
}
//=========================================================================================================


//=========================================================================================================
// raw_to_rh() - Converts a raw reading into relative humidity
//=========================================================================================================
static float raw_to_rh(uint32_t raw_rh)
{   
    // Compute the humidity in 100ths of a percent
    int16_t hundreths = (raw_rh * 10000) >> 16;

    // Return the humidity in %
    return hundreths * .01F;
}
//=========================================================================================================


//=========================================================================================================
// Constructor - Stores the I2C address
//=========================================================================================================
CSHT31::CSHT31(uint8_t i2c_address, sht31_rep_t repeatability)
{
    // Save our I2C address for posterity
    m_i2c_address = i2c_address;

    // Save the command we're going to use to start a measurement
    m_command[0] = (repeatability >> 8);
    m_command[1] = (repeatability & 0xFF);

    // For the moment, we aren't simulating temperatures
    m_is_simulated = false;
}
//=========================================================================================================


//=========================================================================================================
// read_raw() - Reads the raw temperature and relative humidity from the device
//=========================================================================================================
bool CSHT31::read_raw(uint16_t* p_raw_temp, uint16_t* p_raw_rh)
{   
    // Clear the error bit to begin with
    System.error_byte &= ~TEMP_SENSE_ERR;

    // This structure contains fields for the MSB, LSB, and CRC for both temp and humidity
    struct { uint8_t t_msb, t_lsb, t_crc, h_msb, h_lsb, h_crc; } msg;

    // Declare a constant that defines the length of the message we're expecting
    const int MSG_LENGTH = sizeof(msg);

    // We're going to make 3 attempts to read the device
    uint8_t attempts = 3;

    // So long as we haven't exhausted all attempts...
    while (attempts--)
    {
        // Send the command to the device
        if (twi_writeTo(m_i2c_address, m_command, 2, true, true) != 0) continue;

        // Request the temperature and relative humidity from the device
        uint8_t bytes_rcvd = twi_readFrom(m_i2c_address, (uint8_t*)&msg, MSG_LENGTH, true);

        // If we didn't receive all of the bytes we expected, something is awry
        if (bytes_rcvd != MSG_LENGTH) continue;

        //  If both fields had the correct CRC...
        if (msg.t_crc == fast_crc8(&msg.t_msb, 2) && msg.h_crc == fast_crc8(&msg.h_msb, 2))
        {
            *p_raw_temp = msg.t_msb << 8 | msg.t_lsb;
            *p_raw_rh   = msg.h_msb << 8 | msg.h_lsb;
            return true;
        }
    }

    // If we get here, we simply couldn't read the device
    // Set the error bit and tell the caller
    System.error_byte |= TEMP_SENSE_ERR;
    return false;
}
//=========================================================================================================



//=========================================================================================================
// read_c() - Reads the censor and returns the temperature in degrees C
//=========================================================================================================
bool CSHT31::read_c(float *p_temp, float *p_rh)
{
    uint16_t raw_temp, raw_rh;

    // If we're simulating the temperature, make it so
    if (m_is_simulated)
    {
        *p_temp = m_simulated_temp;
        if (p_rh == nullptr) return true;
        p_temp = nullptr;
    }

    // Read the raw values, and complain to the caller if we can't
    if (!read_raw(&raw_temp, &raw_rh)) return false;

    // If the caller wants the temperature, fill it in for them
    if (p_temp) *p_temp = raw_to_c(raw_temp);
    if (p_rh  ) *p_rh   = raw_to_rh(raw_rh);

    // Tell the caller that all is well
    return true;
}
//=========================================================================================================



//=========================================================================================================
// read_f() - Reads the censor and returns the temperature in degrees F
//=========================================================================================================
bool CSHT31::read_f(float* p_temp, float* p_rh)
{
    uint16_t raw_temp, raw_rh;

    // If we're simulating the temperature, make it so
    if (m_is_simulated)
    {
        *p_temp = m_simulated_temp;
        if (p_rh == nullptr) return true;
        p_temp = nullptr;
    }

    // Read the raw values, and complain to the caller if we can't
    if (!read_raw(&raw_temp, &raw_rh)) return false;

    // If the caller wants the temperature, fill it in for them
    if (p_temp) *p_temp = raw_to_f(raw_temp);
    if (p_rh)   *p_rh   = raw_to_rh(raw_rh);

    // Tell the caller that all is well
    return true;
}
//=========================================================================================================


//=========================================================================================================
// simulate_temp() - Pre-define the temperature that read_c() and read_f() will return
//=========================================================================================================
void CSHT31::simulate_temp(float temp)
{
    // Save the temperature we're supposed to simulate
    m_simulated_temp = temp;

    // Keep track of whether we should report real temperatures or simulated ones
    m_is_simulated = (temp > SIM_TEMP_OFF);
}
//=========================================================================================================
